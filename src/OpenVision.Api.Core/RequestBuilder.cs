...
{
    private static readonly ILogger _logger = ApplicationContext.Logger.ForType<RequestBuilder>();
    ...
    /// </summary>
    /// <remarks>The default Value is GET />.</remarks>
    ...
    public void AddParameter(RequestParameterType type, string name, string value)
    {
        ...
        if (value == null)
            _logger.Warning("Add parameter should not get null values. type={0}, name={1}", type, name);
    ...
    /// </summary>
    public HttpRequestMessage CreateRequest() => new(new HttpMethod(Method), BuildUri().ToString());
}